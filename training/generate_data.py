...
ITEM_MAKERS = [make_dallah, make_perfume, make_gold_box, make_electronics,
               make_dates_box, make_wallet, make_watch]
...
def generate_split(n_images, start_id):
    """Generate n_images and save them."""
    for i in range(n_images):
        ...
        # Pick 1-3 random item types
        item_indices = random.sample(range(len(ITEM_MAKERS)),
                                     random.randint(1, min(3, len(ITEM_MAKERS))))

        for cls_id in item_indices:
            item_img = ITEM_MAKERS[cls_id]()
            composed, labels = composite(item_img, result_img, max_instances=2)
            result_img = composed
            for l in labels:
                all_labels.append(f"{cls_id} {l}")
...
if __name__ == "__main__":
    ...
    print(f"\nDone! Generated {N_TRAIN + N_VAL} images with {len(CLASSES)} classes.")
